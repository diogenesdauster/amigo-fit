  ...
  passport.authenticate("local", function (err, user) {
    ...
    if (!user) {
      return res.render("login", {
        error: { message: "Ocorreu um erro ao tentar logar, tente novamente!" },
      });
    }
    ...
    req.logIn(user, function (err) {
      if (err) {
        ...
        return res.render("login", {
          error: {
            message: "Ocorreu um erro ao tentar logar, tente novamente!",
          },
        });
      }
      return res.redirect("/");
    ...
    getIndicacaoBonusUser(token, cpf, function (err, data) {
      if (err) {
        res.render("/", {
          error: {
            message:
              "Ocorreu um erro ao buscar a lista desejada, tente novamente.",
          },
        });
        return;
      }
      ...
      if (data) {
        if (req.params.tipo === "indicados") {
          ...
          res.render("/", {
            error: {
              message: "A lista solicitada não existe, tente novamente.",
            },
          });
          return;
        }
...
app.post("/alterarSenha", function (req, res, next) {
  if (req.query.token) {
    ...
    res.render("alterarsenha", {
      error: "Token Invalido, operação não permitida, tente novamente!!",
    });
  }
});
